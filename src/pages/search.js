...
class Search extends Component {
  ...
  gotoPage = e => {
    const pageId = e.target.className;
    ...
    return (
      <div>
          ...
          >
            ...
            <div style={{ display: 'flex', width: '60vw' }}>
              <input
                ...
                onKeyUp={e => {
                  if (e.keyCode === 13) {
                    window.document.getElementById('search-icon').click();
                  }
                }}
              />
              <i
        ...
        <div
          ...
          style={{ margin: '1em', minWidth: '1240px' }}
        >
          <Query
            query={SEARCH_QUERY}
            variables={{ query: this.props.match.params.search }}
          >
            {({ loading, data, error }) => {
              ...
              return pageData.map(array => (
                <div
                  ...
                  key={`${array.id}-${array.title}`}
                  onClick={this.gotoPage}
                >
                  <div id="others" className={`${array.id}-${array.title}`} />
                  <p id="title" className={`${array.id}-${array.title}`}>
                    {array.title}
                  </p>
                  <div id="subbox" className={`${array.id}-${array.title}`}>
                    <p id="year" className={`${array.id}-${array.title}`}>
                      year&nbsp;
                    </p>
                    <p id="author" className={`${array.id}-${array.title}`}>
                      authorname
                    </p>
                  </div>
                </div>
